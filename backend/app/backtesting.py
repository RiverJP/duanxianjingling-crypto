...
BACKTEST_STRATEGY_VERSION = "2026-07-04v7.1-fee-aware"
STRICT_MIN_QUALITY_SCORE = 78
TREND_MIN_QUALITY_SCORE = 84
RANGE_MIN_QUALITY_SCORE = 84
BASE_INDICATOR_QUALITY = 62
MIN_BACKTEST_RISK_REWARD_RATIO = 1.2
TREND_MIN_RISK_REWARD_RATIO = 1.1
RANGE_MIN_RISK_REWARD_RATIO = 1.5
LIVE_INDICATOR_MIN_QUALITY_SCORE = 82
LIVE_INDICATOR_MIN_RISK_REWARD_RATIO = 1.3
TREND_TARGET_RISK_REWARD_RATIO = 1.5
RANGE_TARGET_RISK_REWARD_RATIO = 1.5
...
TREND_MIN_REWARD_FEE_MULTIPLE = 5.0
RANGE_MIN_REWARD_FEE_MULTIPLE = 6.0
...
def build_backtest_rules(strategy_mode: str, execution_interval: str, trend_interval: str, min_quality_score: int) -> dict:
    return {
        ...
        "entry_conditions": [
            ...
            f"手续费覆盖：趋势单计划止盈金额至少覆盖单笔手续费 {TREND_MIN_REWARD_FEE_MULTIPLE:.0f} 倍，震荡单至少覆盖 {RANGE_MIN_REWARD_FEE_MULTIPLE:.0f} 倍。",
            ...
            f"单个标的每天最多开 {MAX_DAILY_TRADES_PER_ASSET} 单，且平仓后至少冷却 {BACKTEST_SYMBOL_COOLDOWN_SECONDS // 3600} 小时，避免 15m 反复被同一波动扫损。",
            "信号必须有可计算的止盈价和止损价，否则不允许开仓。",
        ],
        "long_logic": [
            "趋势多：1H 做多，4H 不做空，日线不处于下降趋势；15m 回踩EMA后收阳，或突破近6根高点。",
            "区间多：靠近日线/4H 支撑、Fib支撑或震荡里的双底结构；1H 不能强空；15m 必须重新站回EMA20或出现下影反转。",
        ],
        "short_logic": [
            "趋势空：1H 做空，4H 不做多，日线不处于上升趋势；15m 反抽EMA后收阴，或跌破近6根低点。",
            "区间空：靠近日线/4H 阻力、Fib阻力或震荡里的双顶结构；1H 不能强多；15m 必须跌回EMA20或出现上影反转。",
        ],
        "stop_loss_logic": [
            "基础止损距离取最近 20 根执行周期 K 线平均波幅 * 1.6。",
            "最小止损距离不低于入场价的 1.2%。",
            "趋势单止损优先放在最近 12 根15m结构高低点外侧，结构太远则回退为波幅止损。",
            "震荡单止损优先放在大级别支撑/阻力边界外侧。",
        ...
        "take_profit_logic": [
            ...
            f"区间单默认固定盈亏比约 {RANGE_TARGET_RISK_REWARD_RATIO:.1f}:1，且固定止盈空间必须足够覆盖手续费。",
        ...
        "indicator_analysis": [
            f"策略名称：{BACKTEST_STRATEGY_VERSION}。核心思路是先把交易拆成趋势单和震荡单：趋势单用1H主导、4H/日线过滤反向；震荡单必须贴近大级别边界，再用15m确认K线执行，并过滤手续费覆盖不足的小空间交易。",
            ...
            f"量价关系：v7.1 不再把15m放量作为硬过滤；若信号K线成交量达到近20根均量的 {MIN_SIGNAL_VOLUME_MULTIPLE:.2f} 倍以上则加分，不达标只是不加分。",
...
def backtest_asset(
    ...
    for index in range(lookback_candles, len(rows) - 1):
        ...
        if strategy_mode == "indicator":
            plan = build_indicator_trade_plan(asset, rows, index, close, signal_time, trend_filter_rows, market_context, regime_split=True)
            if plan is None:
                continue
        ...
        required_delayed_risk_reward_ratio = TREND_MIN_RISK_REWARD_RATIO if is_trend_strategy_type(strategy_type) else RANGE_MIN_RISK_REWARD_RATIO
        if delayed_risk_reward_ratio < required_delayed_risk_reward_ratio:
            continue
        estimated_fee = round(notional * BACKTEST_FEE_RATE, 2)
        planned_reward_usdt = calculate_planned_reward(str(plan["trade_signal"]), entry_price, take_profit, notional)
        min_reward_fee_multiple = TREND_MIN_REWARD_FEE_MULTIPLE if is_trend_strategy_type(strategy_type) else RANGE_MIN_REWARD_FEE_MULTIPLE
        if planned_reward_usdt < estimated_fee * min_reward_fee_multiple:
            continue
        ...
        entry_reasons.append(
            "v7.1手续费感知回测：趋势单用1H主导、4H/日线过滤反向；震荡单必须贴近大级别边界；"
            f"预期止盈金额约 {planned_reward_usdt:.2f}U，至少覆盖单笔手续费约 {estimated_fee:.2f}U 的 {min_reward_fee_multiple:.0f} 倍；"
            "同一标的平仓后冷却4小时再允许下一单"
        )
        ...
        indicator_snapshot["estimated_fee_usdt"] = estimated_fee
        indicator_snapshot["planned_reward_usdt"] = round(planned_reward_usdt, 2)
        indicator_snapshot["min_reward_fee_multiple"] = min_reward_fee_multiple
        indicator_snapshot["execution_detail"] = (
            "信号K线收盘后确认，下一根15m K线开盘价成交；趋势单用1H主导、4H/日线过滤反向，"
            "震荡单必须贴近边界；日线只读取已完整收盘K线；预期止盈空间必须显著覆盖手续费；"
            "同一标的平仓后冷却4小时。"
        )
...
def build_indicator_trade_plan(
    ...
    long_range_boundary = market_context["near_support"] or market_context["near_fib_support"] or (market_regime == "震荡区间" and market_context["double_bottom"])
    short_range_boundary = market_context["near_resistance"] or market_context["near_fib_resistance"] or (market_regime == "震荡区间" and market_context["double_top"])
    ...
    if regime_split:
        long_trend_setup = one_hour_direction == "做多" and four_hour_direction != "做空" and market_regime != "下降趋势"
        short_trend_setup = one_hour_direction == "做空" and four_hour_direction != "做多" and market_regime != "上升趋势"
        long_range_setup = long_range_boundary and one_hour_direction != "做空" and not short_trend_setup
        short_range_setup = short_range_boundary and one_hour_direction != "做多" and not long_trend_setup

        if long_trend_setup:
            signal = "做多"
            strategy_type = "趋势多"
        elif short_trend_setup:
            signal = "做空"
            strategy_type = "趋势空"
        elif long_range_setup:
            signal = "做多"
            strategy_type = "区间多"
        elif short_range_setup:
            signal = "做空"
            strategy_type = "区间空"
    else:
    ...
    execution_confirmation = build_execution_confirmation(rows, index, signal, market_context, strategy_type, regime_split)
    if execution_confirmation is None:
        return None
    ...
    if regime_split and is_trend_strategy_type(strategy_type):
        ...
        if four_hour_direction == signal:
            ...
            quality -= 12
        quality += 8 if aligned_market else 0
        quality += 3 if (signal == "做多" and dt_break_long) or (signal == "做空" and dt_break_short) else 0
    elif regime_split:
        quality += 8 if one_hour_direction in {signal, "观望"} else -10
        quality += 4 if four_hour_direction in {signal, "观望"} else -8
        quality += 6 if market_regime == "震荡区间" else 0
        quality += 8 if aligned_sr else 0
        quality += 6 if aligned_fib else 0
        quality += 6 if aligned_structure else 0
    else:
        quality += 16 if one_hour_direction == signal else 0
        if four_hour_direction == signal:
            quality += 8
    ...
    required_quality = (
        (TREND_MIN_QUALITY_SCORE if is_trend_strategy_type(strategy_type) else RANGE_MIN_QUALITY_SCORE)
        if regime_split else LIVE_INDICATOR_MIN_QUALITY_SCORE
    )
    ...
    required_risk_reward_ratio = (
        (TREND_MIN_RISK_REWARD_RATIO if is_trend_strategy_type(strategy_type) else RANGE_MIN_RISK_REWARD_RATIO)
        if regime_split else LIVE_INDICATOR_MIN_RISK_REWARD_RATIO
    )
    if risk_reward_ratio < required_risk_reward_ratio:
        return None
    ...
    entry_reasons.extend(str(reason) for reason in execution_confirmation.get("reasons", []))
    entry_reasons.append(f"计划盈亏比为{risk_reward_ratio:.2f}:1，达到{strategy_type}最低 {required_risk_reward_ratio:.1f}:1 开仓要求")
...
def build_execution_confirmation(
    ...
    recent_rows = rows[max(0, index - 60): index + 1]
    if len(recent_rows) < 50:
        return None

    open_price = float(signal_row["open"])
    high = float(signal_row["high"])
    low = float(signal_row["low"])
    close = float(signal_row["close"])
    previous_close = float(previous_row["close"])
    if min(open_price, high, low, close, previous_close) <= 0:
        return None

    recent_20 = rows[max(0, index - 20): index + 1]
    average_range = sum(float(row["high"]) - float(row["low"]) for row in recent_20) / len(recent_20)
    average_range_ratio = average_range / close if close else 0
    if average_range_ratio <= 0 or average_range_ratio > MAX_15M_AVERAGE_RANGE_RATIO:
        return None

    reference_index = max(0, index - 16)
    reference_close = float(rows[reference_index]["close"])
    if reference_close <= 0:
        return None
    recent_move = (close - reference_close) / reference_close
    if signal == "做多" and recent_move > MAX_15M_CHASE_MOVE_RATIO:
        return None
    if signal == "做空" and recent_move < -MAX_15M_CHASE_MOVE_RATIO:
        return None

    closes = [float(row["close"]) for row in recent_rows]
    ema20 = ema(closes, 20)
    ema50 = ema(closes, 50) if len(closes) >= 50 else None
    if ema20 is None:
        return None

    previous_closes = [float(row["close"]) for row in rows[max(0, index - 33): index]]
    previous_ema20 = ema(previous_closes, 20) if len(previous_closes) >= 20 else None
    prior_high = max(float(row["high"]) for row in rows[max(0, index - 6): index])
    prior_low = min(float(row["low"]) for row in rows[max(0, index - 6): index])
    ...
    reasons: list[str] = []
    quality_bonus = 5
    trend_mode = is_trend_strategy_type(strategy_type)
    if signal == "做多":
        ...
        lower_rejection = lower_wick >= max(body * 0.45, average_range * 0.12)
        if trend_mode:
            if not (bullish_close and (reclaim_ema20 or local_breakout or ema_pullback) and (ema_pullback or local_breakout) and upper_wick_ok):
                return None
            reasons.append("趋势多：15m确认K线收阳，并重新站回EMA20、回踩EMA或突破近6根高点")
            reasons.append("趋势多：允许回踩后继续，也允许局部突破，但过滤近16根过度追涨")
        else:
            reversal_confirmed = (reclaim_ema20 or lower_rejection) if regime_split else (reclaim_ema20 or lower_rejection or close > previous_close)
            if not (bullish_close and context_pullback and reversal_confirmed and upper_wick_ok):
                return None
            reasons.append("区间多：价格靠近日线/Fib支撑或双底区域，15m出现收阳反转确认")
            reasons.append("区间多：只在区间下沿附近做，目标看中轴或上方阻力")
    elif signal == "做空":
        ...
        upper_rejection = upper_wick >= max(body * 0.45, average_range * 0.12)
        if trend_mode:
            if not (bearish_close and (lose_ema20 or local_breakdown or ema_retest) and (ema_retest or local_breakdown) and lower_wick_ok):
                return None
            reasons.append("趋势空：15m确认K线收阴，并跌回EMA20、反抽EMA或跌破近6根低点")
            reasons.append("趋势空：允许反抽后继续，也允许局部跌破，但过滤近16根过度追空")
        else:
            reversal_confirmed = (lose_ema20 or upper_rejection) if regime_split else (lose_ema20 or upper_rejection or close < previous_close)
            if not (bearish_close and context_retest and reversal_confirmed and lower_wick_ok):
                return None
            reasons.append("区间空：价格靠近日线/Fib阻力或双顶区域，15m出现收阴反转确认")
            reasons.append("区间空：只在区间上沿附近做，目标看中轴或下方支撑")
    else:
        return None
...
def build_indicator_snapshot(
    ...
    execution_confirmation: dict | None = None,
    regime_split: bool = False,
) -> dict:
    ...
    execution_ema50 = round_optional_price(confirmation.get("ema50"))
    if regime_split:
        trend_filter_detail = (
            f"1H方向={trend_directions.get('1h') or '无'}，4H方向={trend_directions.get('4h') or '无'}；"
            "v7.1趋势单以1H为主且4H不能反向，震荡单允许1H观望但必须贴近边界。"
        )
        risk_plan_detail = (
            ...
            "v7.1趋势单达到1R后保本、达到1.5R后移动止损，震荡单使用固定止盈止损；"
            f"预期止盈空间需覆盖单笔手续费至少 {TREND_MIN_REWARD_FEE_MULTIPLE:.0f} 倍，震荡单至少 {RANGE_MIN_REWARD_FEE_MULTIPLE:.0f} 倍。"
        )
    else:
        trend_filter_detail = (
            f"1H方向={trend_directions.get('1h') or '无'}，4H方向={trend_directions.get('4h') or '无'}；"
            "v6.4以1H为主方向，4H同向加分、观望小加分、反向扣分。"
        )
        risk_plan_detail = (
            f"止损={round_price(stop_loss)}，参考止盈={round_price(take_profit)}，计划盈亏比约{rr:.2f}:1；"
            f"结构止损距离上限为开仓价的{MAX_STRUCTURE_STOP_DISTANCE_RATIO * 100:.0f}%，超出则使用波幅止损；"
            "v6.4确认口径会在信号K线收盘后，以下一根15m开盘价成交，趋势单达到1R后保本、达到1.5R后移动止损。"
        )
    return {
        ...
        "trend_filter_detail": trend_filter_detail,
        ...
        "risk_plan_detail": risk_plan_detail,
...
def build_indicator_exit_prices(
    ...
    trend_mode = is_trend_strategy_type(strategy_type)
    reward_multiple = TREND_TARGET_RISK_REWARD_RATIO if trend_mode else RANGE_TARGET_RISK_REWARD_RATIO

    support = market_context.get("support")
    resistance = market_context.get("resistance")
    if signal == "做多":
        raw_stop = price - stop_distance
        swing_low = min(float(row["low"]) for row in recent[-12:])
        if trend_mode and swing_low < price:
            swing_stop = swing_low * 0.998
            if (price - swing_stop) / price <= MAX_STRUCTURE_STOP_DISTANCE_RATIO:
                raw_stop = min(raw_stop, swing_stop)
        elif support and float(support) < price:
    ...
    raw_stop = price + stop_distance
    swing_high = max(float(row["high"]) for row in recent[-12:])
    if trend_mode and swing_high > price:
        swing_stop = swing_high * 1.002
        if (swing_stop - price) / price <= MAX_STRUCTURE_STOP_DISTANCE_RATIO:
            raw_stop = max(raw_stop, swing_stop)
    elif resistance and float(resistance) > price:
...
def calculate_planned_reward(side: str, entry_price: float, take_profit: float, notional: float) -> float:
    if entry_price <= 0 or take_profit <= 0 or notional <= 0:
        return 0
    if side == "做空":
        reward = (entry_price - take_profit) / entry_price * notional
    else:
        reward = (take_profit - entry_price) / entry_price * notional
