...
def build_live_indicator_trade_plan(db: Session, asset: AssetSnapshot) -> dict[str, float | str | None] | None:
    ...
    if market_context.get("regime") == "数据不足":
        return build_live_empty_plan("v6.4 1H主导指标策略数据不足，暂不进入模拟开仓。", price)
    ...
    if not plan:
        return build_live_empty_plan("v6.4 1H主导指标策略未满足1H方向、15m确认、波动、质量分或盈亏比过滤，当前观望。", price)
    ...
    return {
        ...
        "opportunity_reason": f"v6.4 1H主导指标策略：{strategy_type}。{reason_text}",
    }
...
def apply_live_indicator_context(
    ...
    asset.technical_note = "首页和模拟开仓已切换为 v6.4 1H主导指标策略：15m 执行，1H 决定主方向，4H/日线作为背景加减分；趋势单用参考目标和移动止损，区间单用固定止盈止损。"
