...
def build_live_indicator_trade_plan(db: Session, asset: AssetSnapshot) -> dict[str, float | str | None] | None:
    ...
    if market_context.get("regime") == "数据不足":
        return build_live_empty_plan("v3 指标策略数据不足，暂不进入模拟开仓。", price)
    ...
    if not plan:
        return build_live_empty_plan("v3 指标策略未满足方向、结构、流动性、质量分或盈亏比过滤，当前观望。", price)
    ...
    return {
        ...
        "opportunity_reason": f"v3指标策略：{strategy_type}。{reason_text}",
    }
...
def apply_live_indicator_context(
    ...
    asset.technical_note = "首页和模拟开仓已切换为 v3 指标策略：15m 执行，1H/4H 方向过滤，日线结构确认。"
